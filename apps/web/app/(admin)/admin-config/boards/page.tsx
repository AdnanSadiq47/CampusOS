...
import { BoardListItemDto, ConfigScopeType } from '@campus-os/types';
...
const DEFAULT_BOARDS: BoardListItemDto[] = [
  {
    ...
    code: 'FBISE-PK',
    sortOrder: 1,
    description: 'National Federal examination board for SSC and HSSC qualifications.',
  ...
  },
  {
    ...
    code: 'CAMBRIDGE-UK',
    sortOrder: 2,
    description: 'International curriculum covering IGCSE, O-Level, and A-Level credentials.',
    applyTo: 'ALL_CAMPUSES',
    branchIds: [],
    branchNames: [],
    isActive: true,
  ...
  },
  {
    ...
    code: 'IB-GENEVA',
    sortOrder: 3,
    description: 'Primary Years, Middle Years, and Diploma Programme accreditation.',
  ...
  },
  {
    ...
    code: 'AKUEB-PK',
    sortOrder: 4,
    description: 'Conceptual assessment board for secondary and higher secondary schooling.',
    applyTo: 'ALL_CAMPUSES',
    branchIds: [],
    branchNames: [],
    isActive: true,
...
export default function BoardsPage() {
  ...
  const [editingItem, setEditingItem] = useState<BoardListItemDto | null>(null);
  const [viewingItem, setViewingItem] = useState<BoardListItemDto | null>(null);
  ...
  const handleOpenEdit = (item: BoardListItemDto) => {
    setFormData({
      name: item.name,
    ...
    setItems((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          ...
          showToast(`Board '${item.shortName}' ${nextState ? 'Activated' : 'Deactivated'}`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.shortName.trim()) {
      alert('Please fill Board Name and Short Name.');
      return;
    }
    ...
    if (editingItem) {
      ...
      showToast(`Updated Board '${formData.shortName}'`);
    } else {
      const newItem: BoardListItemDto = {
        id: `b_${Date.now()}`,
        organizationId: '11111111-1111-1111-1111-111111111111',
        name: formData.name.trim(),
        shortName: formData.shortName.trim(),
      ...
      showToast(`Created Board '${newItem.shortName}'`);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminConfigPageHeader
        ...
        title="Boards"
        description="Affiliated educational and national/international examination boards (Federal, Cambridge, IB, AKU-EB)."
        categoryNav={ACADEMIC_SETUP_NAV}
        actionButtonText="Add Board"
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search boards by name, short name (e.g. CAIE, FBISE), code..."
              ...
              {filteredItems.length > 0 ? (
                ...
                <tr>
                  <td colSpan={7} className="py-8 text-center text-xs text-slate-400">
                    No boards found matching your search and filter criteria.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="relative z-10 w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <span>📜</span>
                <span>{editingItem ? 'Edit Board' : 'Add Board'}</span>
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto max-h-[70vh] text-xs">
              <div className="space-y-1">
                <label className="block font-bold text-slate-700 dark:text-slate-300">Board Name *</label>
                <input
                  type="text"
                  required
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  placeholder="e.g. Cambridge Assessment International Education"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  <label className="block font-bold text-slate-700 dark:text-slate-300">Board Code</label>
                  <input
                    type="text"
                    value={formData.code}
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Optional notes regarding board syllabus, examination cycles, or affiliation numbers..."
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                />
              </div>
              ...
              <div className="p-4 -mx-5 -mb-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-end gap-2">
                ...
                >
                  {editingItem ? 'Save Changes' : 'Create Board'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="relative z-10 w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">{viewingItem.shortName}</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-5 space-y-4 text-xs">
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-semibold">Full Board Name</span>
                <p className="font-bold text-slate-900 dark:text-white text-sm mt-0.5">{viewingItem.name}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800">
                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">Board Code</span>
