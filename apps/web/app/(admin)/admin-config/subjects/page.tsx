...
import { BookOpen, FlaskConical, Globe, MapPin } from 'lucide-react';
...
export default function SubjectsPage() {
  ...
  }, [scopedItems, searchQuery, statusFilter, typeFilter, categoryFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Practical & Lab"
          value={scopedItems.filter((i) => i.hasPractical).length}
          icon={<FlaskConical className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
          subtitle="Laboratory enabled"
          variant="info"
        />
        <StatCard
          title="Universal Masters"
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <button
                          ...
                          title="See where this subject is assigned"
                        >
