...
const DEFAULT_LEVELS: AcademicLevelListItemDto[] = [
  {
    id: 'lvl11111-1111-1111-1111-111111111111',
    organizationId: '11111111-1111-1111-1111-111111111111',
    name: 'Early Years / Pre-School',
    shortName: 'EY',
    sortOrder: 1,
    description: 'Foundational years (Playgroup, Nursery, Kindergarten, Reception).',
  ...
  {
    id: 'lvl22222-2222-2222-2222-222222222222',
    organizationId: '11111111-1111-1111-1111-111111111111',
    name: 'Primary Stage',
    shortName: 'PRI',
    sortOrder: 2,
    description: 'Elementary schooling spanning Grades 1 through 5.',
    applyTo: 'ALL_CAMPUSES',
    branchIds: [],
    branchNames: [],
    classCount: 5,
  ...
  {
    id: 'lvl33333-3333-3333-3333-333333333333',
    organizationId: '11111111-1111-1111-1111-111111111111',
    name: 'Middle Stage',
    shortName: 'MID',
    sortOrder: 3,
    description: 'Middle tier classes covering Grades 6, 7, and 8.',
    applyTo: 'ALL_CAMPUSES',
    branchIds: [],
    branchNames: [],
    classCount: 3,
  ...
  {
    id: 'lvl44444-4444-4444-4444-444444444444',
    organizationId: '11111111-1111-1111-1111-111111111111',
    name: 'Secondary / High School',
    shortName: 'SEC',
    sortOrder: 4,
    description: 'Matriculation (Grades 9-10) and Cambridge O-Levels / IGCSE.',
    applyTo: 'ALL_CAMPUSES',
    branchIds: [],
    branchNames: [],
    classCount: 4,
  ...
  {
    id: 'lvl55555-5555-5555-5555-555555555555',
    organizationId: '11111111-1111-1111-1111-111111111111',
    name: 'Higher Secondary / College',
    shortName: 'HSEC',
    sortOrder: 5,
    description: 'Intermediate (FSc, FA, ICom) and Cambridge A-Levels (AS & A2).',
...
export default function AcademicLevelsPage() {
  const [items, setItems] = useState<AcademicLevelListItemDto[]>(DEFAULT_LEVELS);
  ...
  const [editingItem, setEditingItem] = useState<AcademicLevelListItemDto | null>(null);
  const [viewingItem, setViewingItem] = useState<AcademicLevelListItemDto | null>(null);
  ...
  const handleOpenEdit = (item: AcademicLevelListItemDto) => {
    setFormData({
      name: item.name,
    ...
    setItems((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          ...
          showToast(`Level '${item.name}' ${nextState ? 'Activated' : 'Deactivated'}`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      alert('Please fill Academic Level / Stage Name.');
      return;
    }
    ...
    if (editingItem) {
      ...
      showToast(`Updated Academic Level '${formData.name}'`);
    } else {
      const newItem: AcademicLevelListItemDto = {
        id: `lvl_${Date.now()}`,
        organizationId: '11111111-1111-1111-1111-111111111111',
        name: formData.name.trim(),
        shortName: formData.shortName.trim() || null,
      ...
      showToast(`Created Academic Level '${newItem.name}'`);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <AdminConfigPageHeader
        ...
        title="Academic Levels / Stages"
        description="Define pedagogical tiers and educational stages (Early Years, Primary, Middle, Secondary) preceding Classes."
        categoryNav={ACADEMIC_SETUP_NAV}
        actionButtonText="Add Academic Level"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500">Total Stages</span>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500">All-Campus Levels</span>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">Campus-specific stages</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search academic levels by name, short name (e.g. Primary, Secondary)..."
              ...
              {filteredItems.length > 0 ? (
                filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4 font-mono text-slate-600 dark:text-slate-400">{item.shortName || '—'}</td>
                ...
                <tr>
                  <td colSpan={7} className="py-8 text-center text-xs text-slate-400">
                    No academic levels found matching your search and filter criteria.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="relative z-10 w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <span>🪜</span>
                <span>{editingItem ? 'Edit Academic Level' : 'Add Academic Level'}</span>
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto max-h-[70vh] text-xs">
              <div className="space-y-1">
                <label className="block font-bold text-slate-700 dark:text-slate-300">Level / Stage Name *</label>
                <input
                  type="text"
                  required
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  placeholder="e.g. Primary Stage, Secondary Stage, Early Years"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="block font-bold text-slate-700 dark:text-slate-300">Short Name / Code</label>
                  <input
                    type="text"
                    value={formData.shortName}
                    onChange={(e) => setFormData({ ...formData, shortName: e.target.value })}
                    placeholder="e.g. PRI, SEC, EY"
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                  />
                </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Optional notes regarding stage curriculum or grade progression..."
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                />
              </div>
              ...
              <div className="p-4 -mx-5 -mb-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-end gap-2">
                ...
                >
                  {editingItem ? 'Save Changes' : 'Create Academic Level'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="relative z-10 w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
            ...
            <div className="p-5 space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-100 dark:border-slate-800">
                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">Short Code</span>
                  <p className="font-mono font-bold text-slate-800 dark:text-slate-200 mt-0.5">{viewingItem.shortName || '—'}</p>
                </div>
                <div>
